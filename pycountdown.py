...
def draw_h_segment(c, x, y, w, h, segmentwidth, y_offset, fg):
    ...
    margin = 2
    c.create_polygon(x + 0.5 * segmentwidth + margin, y + 0.5 * segmentwidth, x + segmentwidth,
                     y + segmentwidth - margin, x + w - segmentwidth, y + segmentwidth - margin,
                     x + w - 0.5 * segmentwidth - margin, y + 0.5 * segmentwidth, x + w - segmentwidth, y + margin,
                     x + segmentwidth, y + margin, fill=fg)
...
def draw_v_segment(c, x, y, w, h, segmentwidth, x_offset, y_offset, fg):
    ...
    margin = 2
    c.create_polygon(x + 0.5 * segmentwidth, y + margin, x + margin, y + 0.5 * segmentwidth, x + margin,
                     y + h - 0.5 * segmentwidth, x + 0.5 * segmentwidth, y + h - margin, x + segmentwidth - margin,
                     y + h - 0.5 * segmentwidth, x + segmentwidth - margin, y + 0.5 * segmentwidth, fill=fg)
...
def draw_colon(c, x, y, w, h, segmentwidth, fg):
    ...
    c.create_polygon(x + 0.5 * segmentwidth, y, x + segmentwidth, y + 0.5 * segmentwidth, x + 0.5 * segmentwidth,
                     y + segmentwidth, x, y + 0.5 * segmentwidth, fill=fg)
    y = oy + 0.5 * h - 0.5 * segmentwidth + _COLONSPACING
    c.create_polygon(x + 0.5 * segmentwidth, y, x + segmentwidth, y + 0.5 * segmentwidth, x + 0.5 * segmentwidth,
                     y + segmentwidth, x, y + 0.5 * segmentwidth, fill=fg)
...
def draw_time(c, x, y, seconds, drawColon=True, warn=False):
    ...
    if drawColon: draw_number(c, x + 2 * _DIGITWIDTH + 2 * _DIGITSPACING, 0.4 * _DIGITHEIGHT + y, 0.6 * _DIGITWIDTH,
                              0.6 * _DIGITHEIGHT, 0.6 * _SEGMENTWIDTH, ":", fg=(_FG, _FGWARN)[warn])
    draw_number(c, x + 2 * _DIGITWIDTH + 3 * _DIGITSPACING + 0.6 * _DIGITSPACING, 0.4 * _DIGITHEIGHT + y,
                0.6 * _DIGITWIDTH, 0.6 * _DIGITHEIGHT, 0.8 * _SEGMENTWIDTH, seconds, fg=(_FG, _FGWARN)[warn])
